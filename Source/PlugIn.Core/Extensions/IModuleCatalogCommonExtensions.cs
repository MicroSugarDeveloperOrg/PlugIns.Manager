...
public static class IModuleCatalogCommonExtensions
{
    ...
    public static ModuleState GetModuleState<T>(this IModuleCatalog catalog)
        where T : IModule =>
        catalog.Modules.FirstOrDefault(mi => mi.ModuleTypeString == typeof(T).AssemblyQualifiedName).State;
    ...
    public static bool IsInitialized<T>(this IModuleCatalog catalog)
        where T : IModule =>
        catalog.Modules.FirstOrDefault(mi => mi.ModuleTypeString == typeof(T).AssemblyQualifiedName)?.State == ModuleState.Initialized;
